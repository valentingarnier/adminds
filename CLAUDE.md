# Admind — Agent Instructions

## Product Context

**Admind** is an AI-powered tool that helps psychiatrists (initially) in Switzerland write disability insurance reports ("rapports AI" for the Assurance Invalidité system) faster and in compliance with legal requirements.

### The Problem
- Disability insurance reports are highly structured, legally complex documents that take doctors 2-3 hours each
- They follow standardized forms from cantonal AI offices
- Must align with Federal Tribunal case law (e.g. Foerster criteria, structured assessment indicators)
- Reports get sent back if non-compliant
- Doctors are poorly compensated for this work

### Current Wedge
- **Target users**: Psychiatrists in Geneva writing psychiatric AI reports
- **Stage**: Customer discovery phase with 5-10 psychiatrists as beta design partners

### Vision (Venture-Scale)
Become the platform that transforms clinical observations into compliant medico-legal documentation across:
- All report types (disability insurance, accident insurance SUVA/LAA, court reports)
- All medical specialties
- All European markets with similar systems (Germany, Austria, France)

## Overview

This is a SaaS product built with a Next.js frontend, FastAPI backend, Supabase for auth + database, and Stripe for payments. All traffic routes through FastAPI — the frontend never talks to Supabase directly for data.
...
- `NOTIFICATION_EMAIL` — Email address to receive waitlist/contact notifications
